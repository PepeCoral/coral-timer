...
function init() {
    ...
    socket.on('timerUpdate', (newState) => {
        ...
        if (prevState) {
            ...
            if (prevState.timerEndTime < state.timerEndTime) {
                showToast("Timer started");
            }
            // transition from running to paused
            else if (!prevState.timerPauseTime && state.timerPauseTime) {
...
function checkTime() {
    fetch('/time')
        ...
        .then(response => {
            ...
            if (error > 2 * 1000) showToast("Comprueba la configuración de tu reloj, es posible que estes desincronizado", style = "warning", duration = 20000)
